...
const perc = (gain: number, group: string, max: number, minGapMs: number, priority = 0): SfxRule => ({
  bus: 'sfx', group, max, minGapMs, pitchJitter: 0.04, gain, priority,
});
const tonal = (gain: number, group: string, max: number, minGapMs: number): SfxRule => ({
  bus: 'sfx', group, max, minGapMs, pitchJitter: 0.003, gain,
});
const ui = (gain: number): SfxRule => ({ bus: 'ui', group: 'ui', max: 2, minGapMs: 30, pitchJitter: 0.01, gain });

export const SFX_RULES: Record<SfxId, SfxRule> = {
  spin_start: { ...perc(1.2, 'spin', 1, 80), pitchJitter: 0.03 },
  fall_out: perc(1.1, 'fall', 1, 80),
  land_light: perc(0.8, 'land', 3, 30, 1),
  land_medium: perc(0.85, 'land', 3, 30, 2),
  land_heavy: perc(0.9, 'land', 3, 30, 3),
  land_special: { ...perc(0.9, 'landSpecial', 2, 40), pitchJitter: 0.004 },
  ...
  bigwin_tier: { ...tonal(0.75, 'bigwin', 2, 150), duck: { db: -6, holdMs: 1100 } },
  ...
  ui_click: ui(1),
  ui_bet_up: { ...ui(0.75), pitchJitter: 0.002 },
  ui_bet_down: { ...ui(0.75), pitchJitter: 0.002 },
};
