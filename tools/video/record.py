...
def main(argv=None) -> int:
    ...
    ap.add_argument("--kind", default="out", help="id middle part, e.g. frames, stacked, master")
    ...
    if p.is_dir():
        ...
        if not files:
            ...
            return 2
        digest = prov.digest_files(files, p)
    elif p.is_file():
    ...
    row = prov.make_row(
        id=prov.safe_id(a.asset_id or p.stem, a.kind, digest[:8]), path=p, stage=a.stage, sha256=digest,
    ...
    added = prov.record([row], sidecar=a.sidecar, manifest=a.manifest, generated_by=a.model)
    print(json.dumps({"id": row["id"], "sha256": digest, "manifestRowsAdded": added}))
    return 0
