...
/** Dark glass fill shared by every plate. */
const GLASS = 0x05030f;
  ...
  resize(w: number, h: number): void {
    ...
    if (this.shape === 'round') {
      ...
      g.roundRect(-W / 2, -H / 2, W, H, r)
        .fill({ color: GLASS, alpha: this.fillAlpha })
        .stroke({ width: 4, color: 0x000000 });
      g.roundRect(-W / 2 + 6, -H / 2 + 6, W - 12, H * 0.42, r - 4).fill({ color: 0xffffff, alpha: 0.05 });
      g.roundRect(-W / 2 + 6, -H / 2 + 6, W - 12, H - 12, r - 4)
        .stroke({ width: 2.5, color: this.accent, alpha: 0.95 });
    } else {
      g.poly(hex(0))
        .fill({ color: GLASS, alpha: this.fillAlpha })
        .stroke({ width: 3.5, color: 0x000000, join: 'round' });
      // top sheen (flat, hard-edged — no gradient)
      const s = hex(4);
      const sheen = [s[0], s[1], s[2], s[3], s[4] - 2, -H * 0.06, s[10] + 2, -H * 0.06];
      g.poly(sheen).fill({ color: 0xffffff, alpha: 0.07 });
